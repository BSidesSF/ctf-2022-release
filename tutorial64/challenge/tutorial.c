...
void jmprsp();
__asm__(".global jmprsp\njmprsp:\njmp *%rsp");

void challenge() {
  char dest[BUF_SZ];
  printf("Allocated a %d byte buffer at %p\n", BUF_SZ, dest);
  printf("There happens to be a `jmp rsp` instruction at %p.\n", jmprsp);
  ...
  printf("RIP from the calling function is %p and saved at %p.\n", raddr, fp);
...
int main(int argc, char **argv) {
  ...
  printf("Stack cookies, code ASLR, and NX are all disabled.\n");
